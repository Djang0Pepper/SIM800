...
class SIM800( object ):
    ...
    def setup( self ):
        """
        Setup the IO to control the serial port. Extend in subclasses for hardware-specific IO.
        """
    ...
    def getLine( self, timeout=.5, interByteTimeout=.1 ):
        """
        This function gets one line from the modem.
        """
        self._serial.timeout=timeout
        self._serial.inter_byte_timeout=interByteTimeout
    ...
    def waitForLine( self, line ):
        """
        Wait for a single line from the modem.
        """
        while True:
            l = self.getLine()
    ...
    def waitForRing( self ):
        ...
        """
        return self.waitForLine( "RING" )

    
    def answerIncomingCall( self ):
        """
        Convenience method. Answer an incoming call.
        """
        return self.sendATCmdWaitResp( "ATA", "OK" )
